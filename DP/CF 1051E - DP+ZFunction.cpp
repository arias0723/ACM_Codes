...
const int MAXN = 1000005 + 5, MAXNLOG = 25;
...
int main() {
	...
	for (int i = a.length() - 1; i >= 0; --i)
	{
		...
		else if(i + l.length() - 1 < a.length() /*&& i + r.length() - 1 < a.length()*/) {

			int pl = i + l.length() - 1;
			if(zla[i + l.length() + 1] < l.length() && 
					l[zla[i + l.length() + 1]] > a[i + zla[i + l.length() + 1]])
				pl++;

			int pr = i + r.length() - 1;
			if(zra[i + r.length() + 1] < r.length() && 
					r[zra[i + r.length() + 1]] < a[i + zra[i + r.length() + 1]])
				pr--;

			pr = min(pr, (int)a.length() - 1);

			if(pl < a.length() && pr < a.length() && pl <= pr)
				dp[i] = (dpsum[pl + 1] - dpsum[pr + 2] + MOD) % MOD;
