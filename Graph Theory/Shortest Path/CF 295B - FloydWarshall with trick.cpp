...
const int MAXN = 500 + 5, MAXNLOG = 25;
...
// Input
int N, M, Q, P, K;
int G[MAXN][MAXN], D[MAXN][MAXN], X[MAXN];
vll sol;
...
int main() {
	...
	for (int k = 0; k < N; ++k)
	{
		...
		for (int u = 0; u < N; ++u)
		{
			for (int v = 0; v < N; ++v)
			{
				D[u][v] = min(D[u][v], D[u][piv] + D[piv][v]);
			}
		}
	...
	for (std::vector<ll>::reverse_iterator it = sol.rbegin(); it != sol.rend(); ++it)
	{
		cout << *it << " ";
	}
