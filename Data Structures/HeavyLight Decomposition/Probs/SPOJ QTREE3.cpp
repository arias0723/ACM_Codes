...
struct STree {
	...
	void update(int r, int b, int e, const int pos) {
		...
		update(2*r, b, (b+e)/2, pos);
		update(2*r +1, (b+e)/2 +1, e, pos);

		stree[r] = f(stree[2*r], stree[2*r +1]);
	}
	// first 1 in range [B, E]
	int query(int r, int b, int e, const int B, const int E) {

		if(b > e || e < B || b > E) return MAXN;
		if(b >= B && e <= E) {
			return stree[r];
		}
...
struct HLD {
    ...
    HLD(int N) : n(N), sz(N, 1), parent(N, -1), depth(N, 0), head(N), location(N), adj(N), decompChain(N) {
    	stree.build(1, 0, N-1);
    }
    int dfs(int u) {
    	sz[u] = 1;
	...
	int query_upto(int u, int v) {
		...
		res = f( res, stree.query(1, 0, n-1, location[v], location[u]) );
		return res;
	}
    ...
    void update_node(int u) {
        ...
        assert(parent[u] == v);*/
        stree.update(1, 0, n-1, location[u]);
    }
    /*int query_first_1(int l, int u) {
    	int l = LCA(u, v);
...
int main() {
	...
	while(Q--) {
		...
		if(a) {
			int ans = hld.query_upto(b, 0);
			if(ans < N) ans = hld.decompChain[ans] + 1;
			else ans = -1;
			cout << ans << endl;
